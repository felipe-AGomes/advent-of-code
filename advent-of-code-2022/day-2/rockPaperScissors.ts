...
const playWithStrategy = (strategy: StrategyInput) => {
	...
	for (let i = 0; i < arrayStrategyInput.length; i++) {
		const match = arrayStrategyInput[i].split(' ');
		const [playerA, playerB] = match as [ShapeInput, ShapeInput];
		let matchWinner = determineWinner(playerA, playerB);

		if (matchWinner === 'draw') {
			pointsPlayerA += points.draw + shapePoints(playerA);
			pointsPlayerB += points.draw + shapePoints(playerB);
		}

		pointsPlayerA +=
			points[matchWinner === 'playerA' ? 'win' : 'lose'] + shapePoints(playerA);
		pointsPlayerB +=
			points[matchWinner === 'playerB' ? 'win' : 'lose'] + shapePoints(playerB);
	}
